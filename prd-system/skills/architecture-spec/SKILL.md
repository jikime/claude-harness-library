...
> **깊이(lite/full)**: Orchestrator가 **lite**로 지정하면 C4 핵심(Context·Container·Component) + Data·Interface 중심으로 쓰고, ADR은 **되돌리기 어려운 결정이 있을 때만** 남긴다(없으면 "해당 없음"). **full**이면 아래 9섹션 전부. 추적성(모든 FR→컴포넌트)·버전 핀은 깊이와 무관하게 유지한다.
