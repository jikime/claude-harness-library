...
4. **깊이 triage (right-sizing)**: 아이디어 규모로 파이프라인 두께를 정한다.
   - **lite** — 단일 기능·화면 1~2개, 인증/결제/다중역할 없음, 순수 신규 소도구. 핵심 블록만, 게이트 1라운드.
   - **full(기본)** — 다기능·다역할·결제/외부연동·플랫폼 규모. 19블록 + C4 + 게이트 전부.
   - 애매하면 lite로 가정하되 사용자에게 "더 깊게(full) 갈지" 확인한다. 선택을 `idea.md`(또는 README)에 기록하고 각 단계 생성자에게 전달한다. (참고: Required Analysis의 서비스유형·기술필요 판정이 좋은 신호다.)
5. **스택 패밀리 확인**: 이 하네스의 기본 프리셋은 **웹/TypeScript**(Next.js·React·Prisma·Vercel)다. 아이디어가 비-웹(모바일·Python/Go 백엔드·데이터/임베디드)으로 보이면 **silently 가정하지 말고** 스택 패밀리를 사용자에게 묻는다. 정해지면 그 패밀리의 합리적 기본값을 쓰고 PRD §4 Assumptions에 기록한다.
...
## 깊이별 산출 (lite / full)
0단계 triage 결과를 각 단계 생성자에게 전달한다.
- **full**: prd-spec 19블록 전부 · architecture-spec C4 9섹션 + ADR · task-spec 전체. 게이트 최대 2라운드.
- **lite**: 핵심만 — PRD는 §1·2·3·4·7·8·9·10·11·12·13·16·19 중심(해당 없는 §5·6·14·15·17·18은 "해당 없음" 1줄로), ARCHITECTURE는 Context·Container·Component·Data·Interface 핵심 + (되돌리기 어려운 결정 있을 때만)ADR, TASKS는 Operating Guide + 태스크(항상 필요). 게이트 1라운드.
- **lite에서도 불변**: 추적성(FR→컴포넌트→태스크)·EARS·테스트가능 AC·boundaries·가정 기록·사람 승인 게이트는 깊이와 무관하게 유지한다(축소는 분량·선택 블록에 한함).
